...
```
## Confirming that our individual ID variables work
These align with the values on Page 14 of this guide https://www2.census.gov/programs-surveys/sipp/tech-documentation/methodology/2023_SIPP_Users_Guide_OCT24.pdf 

```{r}
monthly %>% group_by(spanel, swave) %>% distinct(ssuid_spanel_pnum) %>% count()

monthly %>% group_by(spanel, swave) %>% distinct(ssuid, pnum) %>% count()

joblevel_wide %>% group_by(spanel, swave) %>% distinct(ssuid_spanel_pnum) %>% count()

## person-level count
monthly %>% distinct(ssuid_spanel_pnum) %>% count()
monthly %>% distinct(ssuid_spanel_pnum, swave) %>% count()

```
Confirming person-months counts match. The ones that should match here do. The 2014 waves all match exactly and then the later panel waves don't show the exact numbers because the table we're comparing to does it by calendar yer
```{r}
monthly %>% group_by(swave,spanel) %>% count() %>% arrange(spanel)

monthly %>% mutate(calyear = case_when(
                             spanel == 2014 & swave == 1 ~ 2013, 
                             ...
                             spanel == 2021 & swave == 4 ~ 2023,
                             
                             spanel == 2022 & swave == 1 ~ 2021,
                             spanel == 2022 & swave == 2 ~ 2022,
                             spanel == 2022 & swave == 3 ~ 2023,
                             
                             spanel == 2023 & swave == 1 ~ 2022,
                             spanel == 2023 & swave == 2 ~ 2023,
                             
                             spanel == 2024 & swave == 1 ~ 2023,
                             .default = NA)) %>% 
  group_by(calyear) %>% distinct(ssuid_spanel_pnum) %>% count() 
```
...
```{r}
joblevel_wide %>% filter(is.na(ejb1_jobid) & !is.na(tjb1_mwkhrs) & !is.na(ejb1_jborse) &
         (!is.na(tjb1_msum) | !is.na(tjb1_prftb))) %>% select(ssuid_spanel_pnum, ssuid, pnum, monthcode, contains("enjflag"), contains("jborse"), contains("jobid"), contains("msum"), contains("prftb"), contains("mwkhrs"), contains("ind")) %>% distinct(ssuid_spanel_pnum) %>% count()
```
...
Then we filter to the working age population based on age at entry into dataset. There are some troublesome age reports in here with people's ages jumping around quite a bit. 
...
temp <- full_data_base %>% 
 ...
 mutate(age_entry = min(tage),
        age_avg = mean(tage),
        age_exit = max(tage),
        age_range = max(tage, na.rm = TRUE) - min(tage, na.rm = TRUE)) %>% 
 filter(age_entry >= 18 & age_entry <= 64) %>% 
 ungroup() %>% 
 relocate(ssuid_spanel_pnum, spanel, swave, tage, age_entry, age_exit)


temp %>% filter(age_range > 5) %>% distinct(ssuid_spanel_pnum) 

temp %>% 
 filter(age_range > 5) %>% 
 select(ssuid_spanel_pnum, spanel, swave, monthcode, tage, age_entry, age_exit) %>% 
...
## Standardizing the unemployment flag (enjflag)
...
```{r reshaping joblevel to long}
rm(monthly, joblevel_wide, full_data_base)
gc()
ptm <- proc.time()
...
proc.time() - ptm
saveRDS(long, "/Volumes/ExtremeSSD/SIPP Data Files/rds/sipp_long_2014_2024.rds")
```



## Creating employment flag
...
long_wd %>% 
 ...
 mutate(some_se = sum(employment_type ==2, na.rm = TRUE),
        ever_se = ifelse(some_se >0,1,0)) %>% 
 ungroup() %>% 
 group_by(ever_se) %>% distinct(ssuid_spanel_pnum, swave) %>% count()
 
```

## Demographic variable consistency checks
...
For example, ~300 people changed their response to whether or not they were hispanic during the course of their survey participation. Likewise, ~1800 people changed their response to their race question during the course of the survey.
...
long_wd <- long_wd %>% 
 ...
 mutate(race_eth = case_when(hispanic_std == 1 ~ "Hispanic",
       ...
       #https://www2.census.gov/programs-surveys/sipp/tech-documentation/methodology/2024_SIPP_Users_Guide.pdf
...
### Parent variable inconsistencies
...
monthly %>% filter(ssuid_spanel_pnum == "114285465212021103") %>% select(spanel, swave, monthcode, tceb, epar_scrnr) # example of someone with inconsistent parent information within the base data.
```
...
```
## Creating first year status flags
Next we need to establish their first year statuses so that we can group people according to how they're entering into SE.
...
```
## Creating consecutive months of unemployment measure
...
gc()
rm(monthly, temp, long, full_data)
library(data.table)
consec_unemp <- as.data.table(long_wd)
...
## Excluding military 
We decided to exclude those who were ever employed as "other" employment type. So I drop those here.
...
```
## More generous unemployment measure y1_status_v2
...
## Quantifying self-employment and work status
...
**This change is implemented below in our filtering to SE samples.**
...
## Finding their first year industry 
```{r}
find_mode <- function(x) {
 # Remove missing values from the vector
 x <- x[!is.na(x)]
 
 # If there are no non-missing values, return NA
 if (length(x) == 0) {
   return(NA)
 }
 
 # Find the mode of the non-missing values
 unique_x <- unique(x)
 tabulated <- tabulate(match(x, unique_x))
 mode_value <- unique_x[which.max(tabulated)]
...
industry_y1 <- long_wd %>% 
 group_by(ssuid_spanel_pnum) %>% 
 mutate(min_swave = min(swave)) %>% 
 ungroup() %>% 
 filter(swave == min_swave) %>% 
 group_by(ssuid_spanel_pnum) %>% 
 summarize(industry_y1_mode = find_mode(industry_fct),.groups = "drop")

```
## Filtering to SE sample
...
se_sample <- long_wd %>% 
 ...
 left_join(industry_y1, by = "ssuid_spanel_pnum")
...
Here is where we use the information we learned above about part-time employment or one-off months with less than full-time employment but people who were continuously employed but not 100% FT. 
...
se_sample.5_10hrs <- se_sample.5_10hrs %>%
 # because we allow for .5 SE here there will be records for jobs that were not SE and will therefore be msising for prtb in that row despite that person having another record with prftb equal to a valid value. So we fill those in before deleting missings here 
 group_by(ssuid_spanel_pnum, calyear) %>% 
 fill(prftb, .direction = "downup") %>% 
 ungroup() %>% 
...
se_sample.5_10hrs_cleanest <- se_sample.5_10hrs %>% 
  group_by(ssuid_spanel_pnum) %>% 
 filter(all(mwkhrs != 0)) %>%
 filter(!any(mwkhrs > 0 & tpearn == 0 & msum == 0 & tptotinc == 0 & prftb == 0)) %>% 
 ungroup()
...
saveRDS(se_sample.5_10hrs, "se_sample_5_10hrs.rds")
saveRDS(se_sample.5_10hrs_cleanest, "se_sample_5_10hrs_cleanest.rds")
```



## Comparing WS and SE 
```{r March 2025 update for descriptives comparing WS and SE 1}
## here we bring in the long_wd from above that isn't filtered to only SE sample, etc. It is filtered to only looking at those who are in the data for at least two years. 
...
```{r}
full_time_period_flags %>% left_join(long_wd, by = "ssuid_spanel_pnum") %>% 
  select(ssuid_spanel_pnum,
     swave,
     spanel,
     shhadid,
...
```{r March 2025 update for descriptives comparing WS and SE 1}
## now with the left-join we'll keep only those we've identified above and get some summary stats on them. 
ws_vs_se <- full_time_period_flags %>% 
 left_join(long_wd, by = "ssuid_spanel_pnum") %>% 
  mutate(has_na = rowSums(is.na(pick(
    monthcode, race_eth_fct_cpsd, sex_fct, immig_fct, 
    educ_fct, industry_fct, parent_std, 
    employment_group, msum, tpearn, married
  ))) > 0) %>%
  # Group and remove entire groups if ANY row in that group was flagged
  group_by(ssuid_spanel_pnum, calyear) %>% 
  filter(!any(has_na)) %>% 
  ungroup() %>%
  select(-has_na)
...
ws_vs_se %>% group_by(ssuid_spanel_pnum) %>% count() %>% ungroup() %>% count(n)

# for consistency we'll get back down to only those we observe for two years with no missingness
ws_vs_se <- ws_vs_se %>% 
  add_count(ssuid_spanel_pnum, name = "total_months") %>% 
...
ws_vs_se <- ws_vs_se %>% 
  mutate(
   # Adjusting for inflation using the following CPI values from the Minneapolis Fed's inflation calculator:
   ...
   tpearn = case_when(
     ...
     calyear == 2017 ~ tpearn*304.7/245.1,
  ...
  msum = case_when(
     calyear == 2013 ~ msum*304.7/233.0,
     calyear == 2014 ~ msum*304.7/236.7,
     calyear == 2015 ~ msum*304.7/237,
     calyear == 2016 ~ msum*304.7/240.0,
     calyear == 2017 ~ msum*304.7/245.1,
     calyear == 2018 ~ msum*304.7/251.1,
     calyear == 2019 ~ msum*304.7/255.7,
     calyear == 2020 ~ msum*304.7/258.8,
     calyear == 2021 ~ msum*304.7/271,
     calyear == 2022 ~ msum*304.7/292.7,
     calyear == 2023 ~ msum,
     .default = NA
   )
 ) 
...
```{r descriptive table comparing ws vs se}
ws_vs_se %>% 
...
temp_earnings <- ws_vs_se %>% 
  ...
  ungroup() %>% 
   mutate(
         ln_tpearn_annual = log_transform(tpearn_annual),
...
person_year <- left_join(temp_earnings, temp, by = c("ssuid_spanel_pnum", "calyear")) %>% 
  mutate(industry_experience = case_when(mode_industry == industry_y1_mode ~ 1,
                                         .default = 0),
         employment_group_fct = factor(employment_group, levels = c("Wage & Salary", "Self-Employed")),
  ...
  set_variable_labels(
    ...
    employment_group_fct = "Employment Type"
  )

```


```{r models comparing WS and SE 2}
library(plm)

tpearn_m1_plm <- plm(ln_tpearn_annual ~ employment_group_fct, 
                    data = person_year, 
...
tpearn_m2_plm <- plm(ln_tpearn_annual ~ employment_group_fct  + 
                   race_eth_fct_cpsd + 
                   educ_fct + 
                   tage + 
                   sex_fct + 
...
tpearn_m7_plm <- update(tpearn_m2_plm, . ~ . + industry_experience)




msum_m1_plm <- plm(ln_msum_annual ~ employment_group_fct, 
                    data = person_year, 
                    index = c("ssuid_spanel_pnum", "calyear"),
                    model = "random",
                    random.method = "walhus")
...
msum_m2_plm <- plm(ln_msum_annual ~ employment_group_fct  + 
                   race_eth_fct_cpsd + 
                   educ_fct + 
                   tage + 
                   sex_fct + 
...
msum_m7_plm <- update(msum_m2_plm, . ~ . + industry_experience)
...
desired_order <- c( 
  # Main Variables of Interest
  "employment_group_fctSelf-Employed",
  "industry_experience",
...
```{r}
coef_map <- build_coef_map_ordered(list(msum_m1_plm,
                                           msum_m2_plm, 
                                           msum_m7_plm, 
                                          tpearn_m1_plm,
                                          tpearn_m2_plm, 
                                          tpearn_m7_plm ), mapping, desired_order)
```
...
# 
ws_vs_se_regressions <- modelsummary(list(tpearn_m1_plm,
                                           tpearn_m2_plm, 
                                           tpearn_m7_plm, 
                                           msum_m1_plm,
                                           msum_m2_plm, 
                                      ...
                                      vcov = "robust",
                                      statistic = "({std.error})",
                                      title = "Table XX Robust SE WS vs SE predicting Log Annual Earnings", 
                                      output = "huxtable",
                                       notes = "Significance levels: + p < 0.1, * p < 0.05, ** p < 0.01, *** p < 0.001")

ws_vs_se_regressions
```


```{r ws vs se table outputs}
library(openxlsx)
wb <- createWorkbook()
addWorksheet(wb, "Wage vs SE earnings")
writeData(wb, ws_vs_se_table, sheet = "Wage vs SE earnings")
saveWorkbook(wb, paste0("ws_vs_se",Sys.Date(),".xlsx"), overwrite = TRUE)
# 
