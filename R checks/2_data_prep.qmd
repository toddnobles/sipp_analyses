...
## Standardizing unemploymnent flag 
...
long_wd <- long_wd %>% 
  filter(months_in_data > 12 ) ## really only == 12 here since we've already filtered to only those with full years. We can do this without grouping because we have 
```
...
se_sample %>%
  select(ssuid_spanel_pnum,
      ...
      thcyincpov,thcyincpovt2, thnetworth ) %>% 
  naniar::gg_miss_var()
```
...
The chunk below calculates the number of people who at some point report a month with work hours but report no income of any sort during that month.
