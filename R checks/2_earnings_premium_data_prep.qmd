---
title: "SIPP Analyses: Earnings Premium Paper Data Prep and Sample Creation"
format: 
  html:
    toc: true
    toc-depth: 5
...
This is the second script in the process for recreating the Stata anaylses for the SIPP data. This script reads in the SIPP files produced by the 1_data_intake.R script, extracts relevant work/income variables, reshapes the person-month-job variables to align to the person-month variables. This script largely mirrors the 2_data_prep.qmd script that runs data prep and cleaning for the unemployment penalty paper. However, this script filters to a sample of individuals who were W&S but then either entered into SE or remained WS. The original Stata version of this was wage_sample_working_paper.do. 
...
```{r reshaping joblevel to long, include = TRUE, eval = FALSE}
...
```{r}
## this file is made in the 2_data_prep.qmd script using the code in the chunk above. 
...
Next we need to establish their frist year statuses so that we can group people according to how they're entering into Wage and Salary.
...
ws_sample <- long_wd %>%
  ...
  filter(mean_hrs >= 15) %>% 
  # now we need to filter to those who were only employed in WS
  filter(ws_months_py1 == months_after_y1)
```
...
ws_sample.5 <- long_wd %>%
  ...
  filter(ws_months_py1/months_after_y1 >=.5)
```
...
ws_sample %>% 
  select(ssuid_spanel_pnum, swave, monthcode, jobid, mwkhrs, msum, tpearn, 
         employment_type, enjflag_std, tptotinc, tage,thnetworth, thval_ast ) %>% head() %>% 
  flextable::flextable()
...
ws_sample %>% 
  select(ssuid_spanel_pnum, swave,jobid,  monthcode, mwkhrs, msum, tpearn, 
         prftb, employment_type, val, tptotinc, tage,thnetworth, thval_ast ) %>%  head() %>% 
  flextable::flextable()
...
Well we have a total of `r ws_sample  %>% distinct(ssuid_spanel_pnum) %>% count()` people in the dataset.
...
```{r, message = FALSE }
ws_sample %>% filter(mwkhrs > 0 & tpearn == 0 & msum == 0 & tptotinc == 0) %>% distinct(ssuid_spanel_pnum) %>% count()
```

Perhaps they're just extraordinarily wealthy? Nope. Even with a modest restriction of networth \< 50000 we still see a few  people with work hours and no income.

```{r}
ws_sample %>% filter(mwkhrs > 0 & tpearn == 0 & msum == 0 & tptotinc == 0  & thnetworth < 50000) %>% distinct(ssuid_spanel_pnum) %>% count()
...
```{r, message = FALSE }
ws_sample %>% filter(mwkhrs == 0) %>% distinct(ssuid_spanel_pnum) %>% count()
ws_sample %>% group_by(ssuid_spanel_pnum) %>% 
  summarize(months_0workhours = sum(mwkhrs == 0), months = n()) %>% 
  arrange(desc(months_0workhours))
```
...
```{r}
ws_sample_cleanest <- ws_sample %>% 
  group_by(ssuid_spanel_pnum) %>%
  filter(all(mwkhrs != 0)) %>% 
  filter(!any(mwkhrs > 0 & tpearn == 0 & msum == 0 & tptotinc == 0)) %>% ungroup()


ws_sample.5_cleanest <- ws_sample.5 %>% 
  group_by(ssuid_spanel_pnum) %>% 
  filter(all(mwkhrs != 0)) %>%
  filter(!any(mwkhrs > 0 & tpearn == 0 & msum == 0 & tptotinc == 0 )) %>% 
  ungroup()
```

# Saving out data

At this point we have multiple potential working datasets. All of the following restrict to working age population and those who we have at least two years of data on (ie can establish based on year 1 how they enter into WS, and then observe at least one year of earnings). Further, they all are also restricted to those individuals that we observe full years of data on, so if someone was missing month 11 in one wave then they get dropped. Put differently, we have to observe each person for all 12 months of the year every year for them to be included. The specific files are described as follows:

-   ws_sample: This file filters to those who were employed for all months post year 1 and who were WS each month after year 1 and who worked an average of at least 15 hours per month.

-   ws_sample.5: This file only differs from the sws_sample in that we loosen the restriction that all months after year 1 had to be WS and allow for only half of the months post year 1 to be WS months for them to be considered in the sample.

-   The ws_sample_cleaned and ws_sample.5_cleaned are built from the above two datasets, but handle the odd cases described immediately above by excluding those who reported months with working hours but no income of any sort as well as those who reported months with no work hours (even though they reported having a job that month since they had a jobid).

```{r}
saveRDS(ws_sample, "ws_sample.rds")
saveRDS(ws_sample.5, "ws_sample_5.rds")
saveRDS(ws_sample_cleanest, "ws_sample_cleanest.rds") 
saveRDS(ws_sample.5_cleanest, "ws_sample_5_cleanest.rds")
```
