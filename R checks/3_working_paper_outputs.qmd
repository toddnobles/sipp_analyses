...
unemp_m2_plm <- plm(ln_tpearn_annual ~ unemp_f12_6  + 
                   ...
                   parent_std +
                   calyear + 
                   ln_thnetworth , data = person_year,
                   index = c("ssuid_spanel_pnum", "calyear"), 
...
attr(rows_to_add, 'position') <- c(1, 3, 5, 8, 12, 14, 16, 18, 21,29)
```
...
## checking that using the walhus from above produces same estimates. IF we don't use walhus above it won't estimate the model for the one-variable model 
# status_m2_plmtest <- plm(ln_tpearn_annual ~ status_f12_6  + 
#                    race_eth_fct_cpsd + 
#                    educ_fct + 
#                    tage + 
#                    age_sq + 
...
status_m3_plm <- update(status_m2_plm, . ~. + mode_industry)
...
status_m6_plm <- update(status_m5_plm, . ~ . -mode_industry)
...
tbl4_status_models_rse <- modelsummary(list(status_m1_plm,
                                           status_m2_plm, 
                                           status_m3_plm, 
                                           status_m4_plm,
                                           status_m5_plm, 
                                      ...
                                      vcov = "robust",
                                      statistic = NULL,
                                      title = "Table 4 Robust SE Y1 Status predicting Log Annual Earnings", 
                                      output = "gt")

tbl4_status_models_rse
addWorksheet(wb, "Tbl 4 Reg Status Earn RSE")
writeData(wb, tbl4_status_models_rse, sheet = "Tbl 4 Reg Status Earn RSE")
```
...
unemp3_m3_plm <- update(unemp3_m2_plm, . ~. + mode_industry)
unemp3_m4_plm <- update(unemp3_m3_plm, . ~ . +industry_experience)
unemp3_m5_plm <- update(unemp3_m4_plm, . ~ . +industry_experience*unemp_f12_3)
unemp3_m6_plm <- update(unemp3_m5_plm, . ~ . -mode_industry)

tbl5_umemp3_models_rse <- modelsummary(list(unemp3_m1_plm,
                                           unemp3_m2_plm, 
                                           unemp3_m3_plm, 
                                           unemp3_m4_plm,
                                           unemp3_m5_plm, 
                                      ...
                                      vcov = "robust",
                                      statistic = NULL,
                                      title = "Table 5 Robust SE Unemp 3 months predicting Log Annual Earnings", 
                                      output = "gt")

tbl5_umemp3_models_rse
...
```{r}
saveWorkbook(wb, "working_paper_outputs_20250813.xlsx", overwrite = TRUE)
```
