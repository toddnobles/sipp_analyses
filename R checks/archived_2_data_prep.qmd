...
long_wd %>% 
 ...
 mutate(some_se = sum(employment_type ==2, na.rm = TRUE),
        ever_se = ifelse(some_se >0,1,0)) %>% 
 ungroup() %>% 
 group_by(ever_se) %>% distinct(ssuid_spanel_pnum, swave, monthcode) %>% count()
...
## here we bring in the long_wd from above that isn't filtered to only SE sample, etc. 
...
to_table <- full_time_period_flags %>% 
...
```{r March 2025 update for descriptives comparing WS and SE 2}
