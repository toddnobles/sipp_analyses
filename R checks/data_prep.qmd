...
```{r}
rm(full_data, full_data_base, long2, joblevel_wide)
```
...
long_wd <- long_wd %>% 
  ...
  mutate(race_eth = case_when(hispanic_std == 1 ~ "Hispanic",
         ...
         race_eth_fct_cpsd = fct_collapse(race_eth_fct, White = "White", Non-white = c("Black", "Asian", "Hispanic", "Other")),
...
```{r, flag for first year unemployment}
# temp <- long_wd %>% 
#   arrange(ssuid_spanel_pnum, swave, monthcode) %>%
#   group_by(ssuid_spanel_pnum) %>%
#   mutate(
#     seq = cumsum(workless_month != lag(workless_month, default = first(workless_month))),
#     seq = if_else(row_number() > 12, NA, seq),
#     seq_length = ifelse(is.na(seq)), cumsum(workless_month)-lag(cumsum(workless_month), default = 0),0)
#   ) %>%
#   ungroup()
# library(dplyr)
# temp <- long_wd %>% 
...
#   mutate(
#     spell = {
#       r <- rle(workless_month)
#       r$lengths <- cumsum(r$lengths)
#       inverse.rle(r) 
#       }
#   )
# 
# temp %>% select(ssuid_spanel_pnum, swave, monthcode, employment_type, workless_month,seq, seq_length) %>% print(n=500)
# # Step 2: Calculate max consecutive unemployment (`max_consec_unempf12`)
# df <- df %>%
#   group_by(ssuid_spanel_pnum_id) %>%
#   mutate(max_consec_unempf12 = max(_seq, na.rm = TRUE)) %>%
#   ungroup() %>%
#   mutate(max_consec_unempf12 = if_else(is.na(max_consec_unempf12), 0, max_consec_unempf12))
```
...
mode_statuses <- long_wd %>% select(ssuid_spanel_pnum, swave, monthcode, employment_type) %>% 
  ...
  summarize(mode_status = find_mode(employment_type)) 

long_wd <- long_wd %>% 
  left_join(mode_statuses, by = "ssuid_spanel_pnum")
```
